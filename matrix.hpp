 ...
 **/
template <typename T> class default_container {
public:
...
/***************************************************************************/
/*******************************MATRIX CLASS********************************/
template <typename T, typename Type = default_container<T>> class matrix {
private:
  Type __MATRIX;
...
template <typename T, typename R1, typename R2> class ADD {
public:
  const R1 &operand1;
  const R2 &operand2;
  std::size_t __size_x;
  ...
  ADD(const R1 &a, const R2 &b)
      : operand1(a), operand2(b), __size_x(operand1.__size_x),
...
//---------------Defining Operator += for two MATRIX---------------------------/
template <typename T, typename R1, typename R2>
...
template <typename T, typename R1, typename R2> class SUB {
public:
  const R1 &operand1;
  const R2 &operand2;
  std::size_t __size_x;
